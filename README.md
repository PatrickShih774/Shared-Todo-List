...
给老板做的多人共享待办清单。Boss + 2 个助理，3人同时使用，跨平台（手机 + PC），实时同步。带完整权限控制：创建者才能编辑/删除，被指派者才能标记完成。
...
| PUT | `/api/todos/:id` | 更新待办 → 广播 `todo:updated`；非创建者编辑返回 **403**，非指派者打完成返回 **403** |
| DELETE | `/api/todos/:id` | 删除待办 → 广播 `todo:deleted`；非创建者返回 **403** |
...
**主界面** → 顶部：当前用户 + 在线状态指示 | 中间：新增表单（仅助理可见）+ 筛选栏 + **默认四象限视图** | 弹窗：编辑待办（仅创建者可打开）

- 移动端优先响应式（触摸目标 ≥ 44px）
- 客户端筛选，无需重复请求
- Socket.io 事件驱动 DOM 实时更新
- 相对时间显示（"2分钟前"）
- **权限控制**：每项待办显示发布者名 👤；编辑/删除按钮仅创建者可见；复选框仅被指派者可点击（未指派则任何人可勾选）
...
- [x] 18. **权限控制**：显示发布者、仅创建者可编辑/删除、仅被指派者可打钩完成
...
| 操作 | 谁可以 | 后端校验 | 前端控制 |
| --- | --- | --- | --- |
| 编辑（非 completed） | 仅创建者 | `created_by` 匹配 → 403 | 隐藏编辑按钮 |
| 删除 | 仅创建者 | `created_by` 匹配 → 403 | 隐藏删除按钮 |
| 打钩完成（有指派者） | 仅被指派者 | `assigned_to` 匹配 → 403 | 禁用 checkbox |
| 打钩完成（无指派者） | 任何人 | 无校验 | 启用 checkbox |

### 前端交互说明

- **列表视图**：每项显示 `👤 创建者名` + `→ 被指派者名`；创建者可见编辑/删除按钮；复选框根据指派关系自动启用/禁用
- **象限视图**：每项卡片显示创建者和指派者信息；编辑/删除按钮仅创建者可见（hover 时出现）；复选框控制同列表视图
- **编辑弹窗**：无权用户点击编辑按钮会弹出 Toast 提示"只有发布者才能编辑"，不会打开弹窗

### 验证方法

1. 打开两个浏览器标签，分别以 Assistant1、Assistant2 登录
2. Assistant1 创建待办 → 指派给 Assistant2
3. Assistant2 标签：能看到 `👤 Assistant1` 发布者名，看不到编辑/删除按钮，复选框可点击
4. Assistant1 标签：能看到编辑/删除按钮，复选框不可点击（因为非指派者）
5. 用开发者工具直接调 `PUT /api/todos/:id` 验证后端 403 拦截
...
14. Assistant1 创建待办指派给 Assistant2 → Assistant2 看到 `👤 Assistant1` 发布者，未见编辑/删除按钮，复选框可点
15. Assistant1 在同一待办上：看到编辑/删除按钮，复选框不可点（非指派者）
16. Assistant2 尝试通过 API 直接 `PUT` 编辑他人创建待办 → 返回 403 `"只有发布者才能编辑"`
17. Assistant2 尝试通过 API 直接 `DELETE` 他人创建待办 → 返回 403 `"只有发布者才能删除"`
18. Assistant1 尝试通过 API 完成指派给 Assistant2 的待办 → 返回 403 `"只有被指派者才能标记完成"`
